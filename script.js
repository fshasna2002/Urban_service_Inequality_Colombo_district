...
/* -------------------------------------------------------------------------
   0b. SUPABASE CONNECTION (Community Feedback)
   ------------------------------------------------------------------------- */
...
const supabaseEnabled = SUPABASE_URL !== 'YOUR_PROJECT_URL' && SUPABASE_KEY !== 'YOUR_PUBLISHABLE_KEY';
const supabaseClient = supabaseEnabled && window.supabase
  ? window.supabase.createClient(SUPABASE_URL, SUPABASE_KEY)
  : null;

const FEEDBACK_TABLE = 'community_feedback';
...
const feedbackMarkers = L.layerGroup().addTo(map);
...
function showFeedbackStatus(message, isError = false) {
  ...
  feedbackStatusEl.className = `feedback-status ${isError ? 'err' : 'ok'}`;
  feedbackStatusEl.hidden = false;
  setTimeout(() => { feedbackStatusEl.hidden = true; }, 4000);
}

// "Select Location" toggle
selectLocationBtn?.addEventListener('click', () => {
  selectingLocation = true;
  selectLocationBtn.classList.add('armed');
  selectLocationBtn.textContent = '📍 Click anywhere on the map…';
});
...
map.on('click', (e) => {
  ...
  feedbackMarkers.clearLayers(); // keep only the current draft pin
  L.marker([selectedLat, selectedLng])
    .addTo(feedbackMarkers)
    .bindPopup(`Selected location<br>${selectedLat.toFixed(5)}, ${selectedLng.toFixed(5)}`)
    .openPopup();

  selectingLocation = false;
  selectLocationBtn.classList.remove('armed');
  selectLocationBtn.textContent = '📍 Location pinned ✓ (click to change)';
});
...
feedbackForm?.addEventListener('submit', async (e) => {
  ...
  if (!supabaseClient) {
    showFeedbackStatus('Feedback storage isn\u2019t configured yet — add your Supabase URL/key in script.js.', true);
    return;
  }

  const record = {
    gn_division: document.getElementById('feedbackGN').value,
    service:     document.getElementById('feedbackService').value,
  ...
  const { error } = await supabaseClient.from(FEEDBACK_TABLE).insert([record]);
  ...
  if (error) {
    console.error(error);
    showFeedbackStatus('Submission failed. Please try again.', true);
    return;
  }

  showFeedbackStatus('Thanks — your feedback was submitted.');
  feedbackForm.reset();
  selectedLat = null;
  selectedLng = null;
  feedbackMarkers.clearLayers();
  selectLocationBtn.textContent = '📍 Select Location';
  loadCommunityFeedback();
});

// Fetch all feedback records and render them
async function loadCommunityFeedback() {
  if (!supabaseClient) {
    document.getElementById('feedbackList').innerHTML =
      '<p class="info-placeholder">Community feedback storage isn\u2019t configured yet.</p>';
    return;
  }
  ...
  if (error) {
    console.error(error);
    document.getElementById('feedbackList').innerHTML =
      '<p class="info-placeholder">Could not load reports.</p>';
    return;
  }
...
// Render feedback both as map markers and as sidebar cards
function displayFeedback(records) {
  feedbackMarkers.clearLayers();

  const panel = document.getElementById('feedbackList');
  panel.innerHTML = '';

  if (records.length === 0) {
    panel.innerHTML = '<p class="info-placeholder">No reports yet — be the first to submit one.</p>';
    return;
  }

  records.forEach(item => {
    if (item.latitude && item.longitude) {
      const marker = L.marker([item.latitude, item.longitude], { title: item.service });
      marker.bindPopup(`
        <div class="popup-title">${item.service}</div>
        <div class="popup-row"><span class="k">GN Division</span><span class="v">${item.gn_division ?? 'N/A'}</span></div>
        <div class="popup-row"><span class="k">Category</span><span class="v">${item.category ?? 'N/A'}</span></div>
        <p style="margin:6px 0 0;font-size:12.5px;">${item.description ?? ''}</p>
      `);
      feedbackMarkers.addLayer(marker);
    }
    ...
    card.innerHTML = `
      <div class="fc-title">${item.service ?? 'Report'}</div>
      <div class="fc-meta">📍 ${item.gn_division ?? 'N/A'} · ${item.category ?? ''} ${dateStr ? '· ' + dateStr : ''}</div>
